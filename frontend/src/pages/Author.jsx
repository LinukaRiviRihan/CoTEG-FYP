...
export default function Author() {
  const author = {
    ...
    bio: "Specializes in Natural Language Processing and Graph Neural Networks. Developer of the CoTEG model's core correlation engine and contextual baseline integrations.",
    initials: "LRR",
    ...
    <div className="min-h-screen px-4 py-12 md:px-6 bg-white font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="max-w-3xl mx-auto">
          {/* Added items-stretch so the right column takes full height if needed */}
          <div className={`bg-white border rounded-2xl p-8 md:p-8 shadow-md flex flex-col md:flex-row items-center md:items-stretch gap-8 md:gap-10 ${author.borderColor}`}>

            {/* Avatar Section */}
            <div className={`w-24 h-24 shrink-0 rounded-full flex items-center justify-center shadow-sm border border-white ring-4 ring-gray-50 overflow-hidden ${!author.imageUrl ? author.themeBg : 'bg-gray-100'}`}>
              {author.imageUrl ? (
                <img
                  src={author.imageUrl}
            ...
            {/* Author Info Section - Added flex flex-col to enable pushing items to the bottom */}
            <div className="flex-1 text-center md:text-left flex flex-col">
              <h3 className="text-2xl font-bold text-zinc-900 mb-1">
                {author.name}
              </h3>
              <p className={`text-sm font-semibold uppercase tracking-wider mb-4 md:mb-6 ${author.themeText}`}>
                {author.role}
              </p>
              ...
              <div className="mt-auto pt-4 flex gap-2 justify-center md:justify-end text-gray-400">
                {/* LinkedIn Link */}
                <a
